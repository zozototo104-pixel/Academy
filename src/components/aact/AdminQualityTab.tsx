...
export function AdminQualityTab() {
  ...
  const [studentThesisFilter, setStudentThesisFilter] = useState('ALL')
  ...
  const [launchHealthOpen, setLaunchHealthOpen] = useState(false)
  const [launchHealth, setLaunchHealth] = useState<any | null>(null)
  const [launchHealthLoading, setLaunchHealthLoading] = useState(false)
  const [dailyTasksOpen, setDailyTasksOpen] = useState(false)
  const [dailyTasks, setDailyTasks] = useState<any | null>(null)
  ...
  const exportSelectedStudent = () => {
    ...
    const link = document.createElement('a')
    link.href = url
    link.download = `student-${safeName}.json`
    document.body.appendChild(link)
    link.click()
    link.remove()
    URL.revokeObjectURL(url)
  ...
  const exportLaunchHealthReport = () => {
    ...
    const link = document.createElement('a')
    link.href = url
    link.download = `launch-health-${new Date().toISOString().slice(0, 10)}.json`
    document.body.appendChild(link)
    link.click()
    link.remove()
    URL.revokeObjectURL(url)
              ...
              <div className="rounded-2xl border border-[#c9a227]/30 bg-[#fffaf0] p-4">
                <div className="flex flex-wrap items-start justify-between gap-2">
                  ...
                  <div className="flex flex-wrap gap-1">
                    ...
                    <Button size="sm" variant="outline" disabled={studentBusyId === selectedStudent.id} onClick={exportSelectedStudent} className="border-[#c9a227]/40 text-xs font-black text-[#a8841a]">تصدير JSON</Button>
