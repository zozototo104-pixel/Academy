...
export function AdminView() {
  ...
  const filteredStudents = useMemo(() => academicStudents.filter((s) => {
    const statusOk = studentStatusFilter === 'ALL' || s.enrollments.some((e) => e.status === studentStatusFilter) || s.latestAdmission?.status === studentStatusFilter
    return statusOk && matchesAdminSearch(studentSearch, [s.name, s.email, s.country, s.latestAdmission?.reference, s.latestAdmission?.program, ...s.enrollments.map((e) => e.program)])
