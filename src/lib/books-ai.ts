...
function makeFallbackMcq(concept: string, specAr: string, caseBased = false, i = 0): GeneratedQuestion {
  ...
  const correctTemplates = [
    `تحويل الفكرة الواردة في الكتاب إلى تحليل للسياق وأصحاب المصلحة والمخاطر قبل القرار`,
    `قراءة الحدث كحالة تتطلب تحديد هدف ونطاق ومسؤوليات ومؤشرات نجاح`,
    `استخلاص نمط القرار أو الصراع في النص وربطه بمنهجية تنفيذ قابلة للمتابعة`,
    `استخدام دلالة المشهد لبناء قرار مهني مبني على الأدلة لا على الانطباع`,
    `تحديد حدود تطبيق الدرس المستفاد من الكتاب قبل تعميمه على مشروع أو مؤسسة`,
  ]
  const distractorBank = [
    `إهمال تفاصيل الكتاب والاكتفاء بتعريف عام في ${specAr}`,
    `اعتبار المشهد مجرد قصة لا علاقة لها بالقرار أو التخطيط أو المخاطر`,
    `اختيار حل جاهز دون فهم الشخصيات أو الصراع أو السياق الوارد في النص`,
    `الاعتماد على رأي شخصي بدلاً من تحليل الدليل السردي ومحتوى الكتاب`,
    `تجاهل أثر العلاقات والتواصل والموارد في تطور الحدث داخل الكتاب`,
    `تعميم الدرس مباشرة على كل المشاريع دون تمييز شروط الحالة وحدودها`,
  ]
