...
function makeFallbackMcq(concept: string, specAr: string, caseBased = false, i = 0): GeneratedQuestion {
  ...
  const stems = caseBased
    ? [
        `تواجه مؤسسة في ${specAr} حالة تتطلب توظيف محور «${topic}» في قرار عملي. أي مسار تحليل هو الأكثر اتساقاً مع مقصد الكتاب؟`,
        `في سيناريو مهني داخل ${specAr} تظهر فيه مشكلة مرتبطة بمحور «${topic}»، أي إجراء أولي يبرهن فهماً تطبيقياً لهذا المحور؟`,
        `عند تحويل محور «${topic}» إلى حالة تقييم في ${specAr}، أي اختيار يحافظ على سلامة القرار قبل التنفيذ؟`,
      ]
    : [
        `أي تفسير يعبّر بدقة عن القيمة التعليمية لمحور «${topic}» عند تطبيقه في ${specAr}؟`,
        `عند دراسة محور «${topic}» في الكتاب، أي ممارسة مهنية تمثل الفهم الأعمق له؟`,
        `أي قرار يعكس توظيفاً منهجياً لمحور «${topic}» داخل مؤسسة تعمل في ${specAr}؟`,
      ]
  const correctTemplates = [
    `تحليل السياق والأطراف والقيود المرتبطة بالمحور قبل اختيار مسار التنفيذ`,
    `ربط المحور بمشكلة عملية قابلة للقياس ثم تحديد قرار ومسؤوليات ومؤشرات متابعة`,
    `استخدام الدليل المستخرج من الكتاب لتفسير الحالة وتحديد حدود التطبيق قبل التعميم`,
    `تمييز العلاقة بين الفكرة المركزية ونتائجها التشغيلية قبل اعتماد الحل`,
    `تحويل المحور إلى سؤال قرار واضح يوازن بين الرهانات والمخاطر والموارد`,
  ]
  const distractorBank = [
    `الاكتفاء بتعريف عام للمحور دون ربطه بمشكلة عملية`,
    `اختيار حل سريع بناءً على الانطباع دون تحليل الأدلة والقيود`,
    `تعميم الفكرة على كل الحالات دون اختبار شروط التطبيق`,
    `فصل القرار التنفيذي عن السياق الذي عالجه الكتاب`,
    `التركيز على المصطلح مجرداً وإهمال علاقته بالنتائج`,
    `تأجيل تحليل المخاطر إلى ما بعد التنفيذ بدلاً من إدخالها في القرار`,
  ]
  ...
  return {
    ...
    cognitiveSkill: caseBased ? 'APPLY' : 'ANALYZE',
    difficulty: caseBased ? 'MEDIUM' : 'ADVANCED',
    correctRationale: `الإجابة الصحيحة تربط محور «${topic}» بسياقه التطبيقي وحدود استخدامه، بدلاً من التعامل معه كمعلومة معزولة.`,
    points: 2,
  }
}
...
- صياغة عربية فصحى واضحة ودقيقة علمياً؛ صحّح تشوهات OCR في صياغة السؤال ولا تنقل عبارات مكسورة مثل «مستنداً خصائصيه» أو تراكيب غير عربية.
