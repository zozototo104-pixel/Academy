...
function makeFallbackMcq(concept: string, specAr: string, caseBased = false, i = 0): GeneratedQuestion {
  ...
  const stems = caseBased
    ? [
        `بالاستناد إلى هذا الحدث/الفكرة من الكتاب: «${idea}»، ما القرار الأكثر اتساقاً مع ممارسة مهنية ناضجة داخل تخصص ${specAr}؟`,
        `إذا اعتبرنا الموقف الآتي من الكتاب حالة تطبيقية في ${specAr}: «${idea}»، فما التصرف الأدق؟`,
        `ما القراءة المهنية الأقرب لمشهد/فكرة «${idea}» عند إسقاطها على واقع ${specAr}؟`,
      ]
    : [
        `أي عبارة تفسر بصورة أدق دلالة «${idea}» من الكتاب عند ربطها بتخصص ${specAr}؟`,
        `كيف يمكن فهم فكرة «${idea}» من الكتاب كدرس مهني في ${specAr}؟`,
        `ما الاستنتاج الأكثر صحة من الفكرة الآتية في الكتاب: «${idea}» عند تطبيقها على ${specAr}؟`,
      ]
