...
interface DegreeAcademicPolicy {
  levelPolicy: string
  readingDepth: string
  assessmentOrientation: string
...
function academicPolicyForCategory(category: string): DegreeAcademicPolicy {
  if (category === 'DOCTORATE') {
    return {
      levelPolicy: 'مستوى دكتوراه مهنية: مراجع بحثية متقدمة، أطر نظرية نقدية، منهجية بحث، نماذج تقييم، ودراسات حالة مركبة لا تصلح للمدخل العام.',
      readingDepth: 'قراءة نقدية معمقة: مقارنة مدارس فكرية، تحليل فجوات، نقد منهجيات، وربط مباشر ببحث التخرج والممارسة المهنية العليا.',
      assessmentOrientation: 'امتحانات يغلب عليها المقال التحليلي، التقييم النقدي، الحالة المركبة، وصياغة توصيات مبنية على دليل ومنهجية.',
      suggestedSemesters: [1, 2],
      examInstruction: 'ارفع مستوى الأسئلة إلى التحليل والتقييم النقدي؛ اجعل أسئلة الدكتوراه تقيس القدرة على نقد نموذج أو منهجية أو قرار وربطه بنتائج بحثية وتطبيق مهني.',
    }
  }
  if (category === 'MASTERS') {
    return {
      levelPolicy: 'مستوى ماجستير مهني: مراجع متخصصة تجمع النظرية الحديثة، النماذج التطبيقية، حالات الأعمال/المهنة، ومهارات اتخاذ القرار.',
      readingDepth: 'قراءة تحليلية تطبيقية: فهم المفاهيم، تطبيقها على حالات، المقارنة بين الأدوات، وبناء حلول مهنية مبررة.',
      assessmentOrientation: 'امتحانات متوازنة بين MCQ المتخصص، أسئلة قصيرة تحليلية، حالات عملية، ومقالات محدودة مرتبطة بالكتب.',
      suggestedSemesters: [1, 2],
      examInstruction: 'اجعل مستوى الأسئلة تحليلياً وتطبيقياً؛ ركّز على الحالة المهنية وربط المفهوم بالقرار والأثر، لا على الحفظ فقط.',
    }
  }
  if (category === 'DIPLOMA') {
    return {
      levelPolicy: 'مستوى دبلوم مهني: مراجع تأسيسية عملية، لغة واضحة، أدوات قابلة للتطبيق، أمثلة مباشرة، وتمارين تشغيلية.',
      readingDepth: 'قراءة تأسيسية تطبيقية: تعريف المفاهيم الأساسية، خطوات العمل، النماذج العملية، والأخطاء الشائعة في الممارسة.',
      assessmentOrientation: 'امتحانات تركّز على الفهم والتطبيق المباشر، اختيار من متعدد وصح/خطأ وسيناريوهات قصيرة، مع عدد أقل من المقالات الثقيلة.',
      suggestedSemesters: [1, 2],
      examInstruction: 'اجعل الأسئلة عملية وواضحة وتقيس فهم الأساسيات وتطبيقها على مواقف قصيرة؛ لا ترفعها إلى نقد بحثي متقدم إلا عند الحاجة.',
    }
  }
  if (category === 'ACCREDITATION' || category === 'INTL_CERT') {
    return {
      levelPolicy: 'مستوى اعتماد/شهادة مهنية: مراجع معيارية، أدلة كفايات، أطر امتثال، قوائم تحقق، وسيناريوهات تحقق من الجاهزية المهنية.',
      readingDepth: 'قراءة معيارية موجهة للكفايات: مطابقة المعايير، فهم المتطلبات، وإثبات الجاهزية المهنية بوثائق وأمثلة.',
      assessmentOrientation: 'تقييم يعتمد على سيناريوهات معيارية، أسئلة تحقق من الكفاية، حالات امتثال، ومهام عملية قصيرة.',
      suggestedSemesters: [1],
      examInstruction: 'اجعل الأسئلة مرتبطة بمعايير الكفاية والتحقق من الجاهزية المهنية؛ ركّز على الاختيار الصحيح ضمن سيناريو امتثال أو اعتماد.',
    }
  }
  return {
    levelPolicy: 'مستوى مهني عام: مراجع متخصصة متدرجة من التأسيس إلى التطبيق حسب طبيعة البرنامج.',
    readingDepth: 'قراءة مهنية منظمة: مفاهيم، نماذج، تطبيقات، وحالات عملية.',
    assessmentOrientation: 'تقييم متوازن بين الفهم والتطبيق والتحليل وفق طبيعة التخصص.',
    suggestedSemesters: [1, 2],
    examInstruction: 'اضبط مستوى الأسئلة حسب الدرجة والتخصص ولا تجعلها عامة.',
  }
}
...
function fallbackBookSuggestions(program: { titleAr: string; titleEn?: string | null; category: string; description?: string | null }): BookSuggestion[] {
  ...
  return base.map(([titleEn, author, year, reason], index) => ({
    ...
    link: '',
    referenceLink: googleBooksSearch(`${titleEn} ${topic}`),
    linkType: 'MISSING_DIRECT_LINK',
    linkReadHint: 'اقتراح احتياطي بلا رابط قراءة مباشر؛ ارفع ملف الكتاب أو ضع رابط PDF/TXT/HTML مفتوح قبل بناء بنك المعرفة والامتحان.',
...
سياسة المستوى الأكاديمي لهذه الدرجة:
- سياسة اختيار الكتب: ${policy.levelPolicy}
- عمق القراءة المطلوب: ${policy.readingDepth}
- طبيعة الامتحان المتوقعة: ${policy.assessmentOrientation}
...
- لا تقترح نفس نوع الكتب لكل الدرجات: كتاب الدبلوم يجب أن يكون عملياً تأسيسياً، وكتاب الماجستير تحليلياً تطبيقياً، وكتاب الدكتوراه بحثياً نقدياً متقدماً.
- أضف لكل كتاب semester مقترحاً من القيم ${policy.suggestedSemesters.join(' أو ')}، وlevelPolicy، وreadingDepth، وassessmentOrientation بصياغة موجزة مخصصة لهذا الكتاب.
...
- لكل كتاب حاول توفير directReadableLink حقيقي فقط: رابط PDF مباشر، TXT مباشر، صفحة HTML مفتوحة فيها نص الكتاب/الفصل، أو مستودع قراءة مفتوح مثل OpenStax/Pressbooks/Project Gutenberg/LibreTexts/OAPEN/NIST.
- ممنوع وضع Google Books أو Goodreads أو WorldCat أو نتيجة بحث في directReadableLink؛ هذه روابط فهرسة/معاينة وليست نص كتاب.
- إذا لم تكن متأكداً من وجود رابط قراءة مباشر قانوني ومفتوح، اجعل directReadableLink فارغاً "" وضع Google Books أو Open Library في referenceLink فقط للتحقق اليدوي.
- لا تخترع روابط PDF؛ الرابط المباشر يجب أن يبدو قابلاً للفتح والقراءة فعلاً.

أجب بصيغة JSON فقط بدون أي نص إضافي — مصفوفة من 8 عناصر:
[{"title":"<العنوان بالعربية>","titleEn":"<العنوان بالإنجليزية>","author":"<المؤلف>","year":"<سنة>","reason":"<سبب الاختيار>","directReadableLink":"<رابط قراءة مباشر فقط أو فارغ>","referenceLink":"<رابط تحقق/فهرسة اختياري لا يستخدم للقراءة>","semester":1,"levelPolicy":"<لماذا يناسب هذا الكتاب درجة ${level}>","readingDepth":"<كيف يقرأه الطالب في هذا المستوى>","assessmentOrientation":"<طبيعة الأسئلة أو التقييم المبني عليه>"}]`
...
function stripExamKnowledgeMeta(value: unknown, max = 1000): string {
  return cleanText(value, max)
    ...
    .replace(/^\s*\d+\.\s*/u, '')
    .replace(/^\s*(?:مفهوم|تعريف|خلاصة|ملخص|مصطلح|بذرة\s+سؤال|سؤال|نظرية\s+أو\s+إطار|نظرية|إطار|منهجية|طريقة|حالة\s+تطبيقية)\s*(?:من\s+النص|مستخرجة\s+من\s+النص|مستخرج\s+من\s+النص|من\s+الكتاب|مستخرجة\s+من\s+الكتاب|مستخرج\s+من\s+الكتاب)?\s*[:：\-–—]+\s*/giu, '')
    ...
    .replace(/(?:^|[\n\s.؛،-])(?:محور\s+معرفي\s+مهم|الفكرة|خلاصة\s+أكاديمية|خلاصة\s+اكاديمية|مقتطف\s+داعم|دليل\s+من\s+المحتوى|دليل\s+من\s+المحتوي|كلمات\s+مفتاحية|مصطلحات\s+مرتبطة|مصدر\s+القراءة|جودة\s+المحتوى|جودة\s+المحتوي|note|sourceNote|contentQuality|bookEvidence|sourceBookTitle|sourceChapter|sourceLocator)\s*[:：]\s*/giu, ' ')
...
function bookToSpecialtyBridgeInstruction(books: ExamSourceBook[], specialtyAr: string): string {
  if (!likelyNarrativeOrReflectiveBook(books)) {
    return `هذه الكتب قد تكون تخصصية أو مهنية. استخرج المفاهيم والنماذج والمنهجيات منها مباشرة، ثم اربطها بتطبيقات ${specialtyAr}. حتى في هذه الحالة يجب أن يحمل كل سؤال دليلاً واضحاً من الكتاب نفسه، لا سؤالاً عاماً في التخصص.`
  }
...
- يجب أن يظهر في نص السؤال أثر مباشر من الرواية: موقف، شخصية، صراع، قرار، نتيجة، أو عبارة دلالية. لا تكتب سؤالاً يبدأ من نظرية إدارة مشاريع مجردة.
- لا تستخدم أسماء مراجع خارجية مثل PMBOK أو Kerzner أو Agile إلا إذا ظهرت صراحة في نص الكتاب أو كان السؤال يطلب إسقاط حدث من الكتاب عليها دون ادعاء أنها واردة فيه.
...
function bookSourceNorm(books: ExamSourceBook[]): string {
  ...
  // نطبّق تنظيف OCR نفسه المستخدم في بناء الأدلة حتى لا تُرفض أسئلة صحيحة بسبب اختلاف «يف/في» أو أخطاء قراءة بسيطة.
  return norm(books.map((b) => cleanAcademicGeneratedText(sanitizeExamText(b.textContent || '', EXAM_BOOK_MAX_CHARS), EXAM_BOOK_MAX_CHARS)).join('\n'))
}
...
function fallbackExamConcepts(
  ...
  const directBookChunks = books.flatMap((book) => {
    const title = cleanText(book.title, 90)
    const full = cleanAcademicGeneratedText(sanitizeExamText(book.textContent || '', EXAM_BOOK_MAX_CHARS), EXAM_BOOK_MAX_CHARS)
    if (full.length < 180) return []
    return [0, 0.12, 0.24, 0.36, 0.48, 0.6, 0.72, 0.84, 0.96]
      .map((ratio) => stripExamKnowledgeMeta(pickWindow(full, ratio, 520), 520))
...
function makeFallbackMcq(concept: string, specAr: string, caseBased = false, i = 0): GeneratedQuestion {
  ...
  const stems = caseBased
    ? [
        `يعرض الكتاب المقطع الآتي: «${idea}». عند تحويله إلى حالة مهنية في ${specAr}، أي قرار هو الأكثر انضباطاً؟`,
        `أمام مدير يعمل في ${specAr} موقف مستمد من الكتاب: «${idea}». ما التصرف الذي يحافظ على سلامة التحليل قبل التنفيذ؟`,
        `في حالة تطبيقية مستندة إلى المقطع «${idea}»، ما القراءة المهنية التي تمنع القرار المتسرع داخل ${specAr}؟`,
      ]
    : [
        `وفق المقطع الآتي من الكتاب: «${idea}»، أي استنتاج مهني هو الأدق في سياق ${specAr}؟`,
        `يعالج الكتاب الفكرة الآتية: «${idea}». أي تطبيق عملي يعكسها بصورة صحيحة في ${specAr}؟`,
        `بالاعتماد على الدليل النصي «${idea}»، ما القرار أو التحليل الأنسب عند استخدامه في ${specAr}؟`,
      ]
  const anchor = idea.length > 80 ? `${idea.slice(0, 80)}…` : idea
  const correctTemplates = [
    `بناء تحليل خاص بفكرة «${anchor}» يحدد السياق والأطراف والمخاطر قبل القرار`,
    `تحويل «${anchor}» إلى حالة تتطلب هدفاً واضحاً ومسؤوليات ومؤشرات نجاح`,
    `استخلاص نمط القرار أو الصراع في «${anchor}» وربطه بخطوات تنفيذ قابلة للمتابعة`,
    `استخدام دلالة «${anchor}» لصناعة قرار مهني مبني على دليل من الكتاب لا على الانطباع`,
    `تحديد حدود تطبيق درس «${anchor}» قبل تعميمه على مشروع أو مؤسسة`,
  ]
  const distractorBank = [
    `إهمال خصوصية «${anchor}» والاكتفاء بتعريف عام في ${specAr}`,
    `اعتبار «${anchor}» مجرد تفصيل سردي لا يحمل أي دلالة قرار أو تخطيط`,
    `اختيار حل جاهز دون تحليل السياق والشخصيات أو الأطراف المرتبطة بـ«${anchor}»`,
    `الاعتماد على رأي شخصي بدل تفسير الدليل النصي الظاهر في «${anchor}»`,
    `تجاهل أثر العلاقات والتواصل والموارد في تطور «${anchor}» داخل الكتاب`,
    `تعميم درس «${anchor}» مباشرة دون تمييز شروط الحالة وحدودها`,
  ]
...
export async function generateExamQuestionBatch(
  ...
  const bridgeInstruction = bookToSpecialtyBridgeInstruction(evidenceBooks, specialty.ar)
...
سياسة مستوى الدرجة التي يجب أن تضبط نوع الكتب وقوة الامتحان:
- سياسة القراءة: ${policy.readingDepth}
- طبيعة التقييم: ${policy.assessmentOrientation}
- تعليمات الامتحان حسب الدرجة: ${policy.examInstruction}
...
النافذة المطلوبة الآن (${requestedPlan.length} سؤالاً من الدفعة ${batchIndex + 1}، ابتداءً من ترتيب ${normalizedOffset + 1}):
مواصفة الدفعة الكاملة: ${spec.instruction}

التوزيع الإلزامي الدقيق لهذه النافذة فقط:
...
- كل سؤال يجب أن يبدأ من دليل واضح من الكتاب نفسه: حدث، شخصية، صراع، قرار، فكرة، مقطع، فصل، أو مثال ورد في النص، ثم يُسقط هذا الدليل على تخصص ${specialty.ar}.
- لا تجعل نص السؤال نسخة حرفية مشوهة من بنك المعرفة. اكتب سؤالاً تعليمياً كاملاً وواضحاً، ثم ضع الدليل الخام في bookEvidence فقط.
- ممنوع أن يظهر داخل السؤال أو الخيارات أو الإجابة أي من العبارات الداخلية: «محور معرفي مهم»، «الفكرة:»، «دليل من المحتوى»، «كلمات مفتاحية»، «بنك المعرفة الأكاديمي المستخرج من الكتب».
- تجنّب الصيغ العامة الضعيفة مثل: «كيف يمكن فهم فكرة...» أو «أي عبارة تفسر بصورة أدق دلالة...» ما لم تضف حالة واضحة وسياقاً مهنياً محدداً.
...
- ممنوع تماماً صياغة سؤال عن «رابط الكتاب» أو «مصدر الرابط» أو ملاحظة أن النص لم يظهر؛ هذه بيانات تقنية وليست مادة علمية.
- ممنوع ظهور أي رموز تقنية داخل نص السؤال أو الخيارات أو الإجابة مثل: CONCEPT أو THEORY أو METHOD أو CASE أو DEFINITION أو QUESTION_SEED أو SUMMARY أو عبارة «أهمية 60». هذه ملاحظات داخلية وليست جزءاً من الامتحان.
