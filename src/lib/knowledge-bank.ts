...
// توافق احتياطي فقط مع أي build سابق؛ لا نستخدم هذه القيم لتقييد عدد عناصر Gemini.
const PREFERRED_AI_MIN_ITEMS = MIN_ACCEPTABLE_AI_ITEMS
const PREFERRED_AI_MAX_ITEMS = MAX_ITEMS_PER_BOOK
...
استخرج كل عناصر المعرفة العميقة والمختلفة فعلاً التي تراها مهمة من محتوى الكتاب. لا تلتزم بعدد محدد؛ الكتاب هو الذي يحدد حجم البنك. لا تضغط الكتاب في عناصر قليلة: تعامل مع كل باب/فصل/عنوان رئيسي، وكل نظرية أو نموذج أو مصطلح أو منهجية أو حالة مستقلة، كعنصر مستقل إذا كان له معنى معرفي واضح. لا تدمج عدة أفكار مستقلة داخل عنصر واحد، ولا تضف عناصر متشابهة لإكمال رقم معين. اجعل كل عنصر مركزاً غير مطول حتى يغطي البنك كل المحاور الجوهرية. كل عنصر يجب أن يحتوي:
...
أجب JSON فقط كمصفوفة عناصر معرفة عميقة ومختلفة فعلاً حسب ما يستحقه محتوى الكتاب، دون الالتزام بعدد محدد. لا تضغط الكتاب في عناصر قليلة: تعامل مع كل باب/فصل/عنوان رئيسي، وكل نظرية أو نموذج أو مصطلح أو منهجية أو حالة مستقلة، كعنصر مستقل إذا كان له معنى معرفي واضح. لا تدمج عدة أفكار مستقلة داخل عنصر واحد، ولا تكرر الفكرة بصياغات متعددة، ولا تكمل العدد بعناصر عامة. كل عنصر بهذا الشكل:
...
أنشئ عناصر معرفة موزعة بذكاء على هذه الفئات حسب ما يسمح به توصيف الكتاب والبرنامج، دون الالتزام بعدد محدد:
...
طريقة العمل:
- اقرأ هذه الوحدة فقط، ومرّ على نقاط التغطية المستخرجة من النص نقطة نقطة حتى لا يتوقف التحليل عند أول فكرة.
- قسّمها داخلياً إلى أفكارها الفعلية: مفاهيم، تعريفات، نظريات/نماذج، منهجيات، حالات، وبذور أسئلة.
- لكل فكرة مستقلة ومهمة أنشئ عنصر معرفة مستقل، ولو كان الباب الواحد يحتوي عدة عناوين أو نماذج أو مصطلحات فاستخرج مجموعة عناصر لهذا الباب.
- إذا وجدت فقرة أو نقطة مشوهة أو غير مفهومة داخل الوحدة فتجاوزها فقط وانتقل للنقطة التالية، ولا توقف تحليل باقي الوحدة.
