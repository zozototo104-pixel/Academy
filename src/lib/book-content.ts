...
- إذا كان الملف رواية أو قصة أو نصاً سردياً: استخرج الشخصيات، الأحداث، الصراعات، القرارات، التحولات، العلاقات، المشاهد المفصلية، والرسائل الضمنية؛ لأنها ستُستخدم لاحقاً كحالات دراسية تُسقط على تخصص الطالب.
- اجعل الناتج طويلاً ومنظماً بما يكفي ليصبح قاعدة معرفة للامتحان والمشرف الذكي.
- اكتب الناتج بالعربية الأكاديمية السليمة حتى لو كان الكتاب أجنبياً.
- تجاهل أي سطر OCR مكسور أو مقلوب أو مختلط اللغة؛ لا تنقل كلمات مثل «يف»، «اختاذ»، «القررا»، «كتاب اطلب»، ولا سنوات غير منطقية مثل 8121.
...
export async function hydrateBookContentForExam(book: RawBookForHydration): Promise<HydratedExamBook> {
  ...
  const hasStoredFileRef = !!book.data || !!book.storageKey || !!book.fileUrl
  const hasReadableBackingSource = hasStoredFileRef || !!book.link
  ...
  if (book.link) {
    ...
    if (storedFallbackAllowed) {
      return returnStored(`النص المخزن سابقاً قصير لكنه صالح مؤقتاً بعد تعذر إعادة قراءة رابط الكتاب: ${fromLink.note}`)
    }
