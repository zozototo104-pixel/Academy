...
export async function POST(req: NextRequest) {
  try {
    ...
    await audit(
      ...
      isDocumentReplacement ? 'RESUBMIT_ADMISSION_DOCUMENTS' : isServiceRequest ? 'SUBMIT_SERVICE_REQUEST' : 'SUBMIT_ADMISSION',
      ...
      isDocumentReplacement
        ...
        : isServiceRequest
          ...
          : `${app.reference} — ${selectedTitle} — مستندات: ${(app.files || []).length}/4 + إقرار — فاتورة رسوم تقديم ${appFee}import { NextRequest, NextResponse } from 'next/server'
import { db } from '@/lib/db'
import { getCurrentUser } from '@/lib/auth'
import { ADMISSION_FEES } from '@/lib/academyData'
import { getSettingNum, nextInvoiceNo } from '@/lib/settings'
