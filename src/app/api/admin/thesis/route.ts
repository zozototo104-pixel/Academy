...
import { emailDefenseScheduled, emailThesisPlanDecision, emailThesisResultApproved } from '@/lib/mailer'
  ...
  try {
    ...
    if (action === 'APPROVE_PLAN') {
      ...
      const updated = await db.thesisSubmission.update({
        where: { id },
        data: { status: 'PLAN_APPROVED', reviewNote: safeReviewNote || null, reviewedAt: new Date() },
      })
      await notify(
        thesis.userId,
        'THESIS',
