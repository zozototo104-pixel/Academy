...
export async function POST() {
  try {
    ...
    steps.push({ name: 'منع امتحان الفصل الثاني قبل اكتمال الالتزام المالي', ok: academicJourney.tuitionGate.semester2BeforeFinalPayment.ok === false && academicJourney.tuitionGate.semester2BeforeFinalPayment.code === 'TUITION_FULL_REQUIRED', detail: academicJourney.tuitionGate.semester2BeforeFinalPayment.code || 'OK', data: academicJourney.tuitionGate.semester2BeforeFinalPayment })
    steps.push({ name: 'السماح بامتحان الفصل الثاني بعد سداد بقية التقسيط', ok: academicJourney.tuitionGate.semester2AfterFinalPayment.ok === true, detail: `paid=${academicJourney.tuitionGate.planAfterFinal?.paidTuition || 0}`, data: academicJourney.tuitionGate.planAfterFinal })
