...
import { fallbackExamQuestionBatch, generateExamQuestionBatch, EXAM_BATCH_COUNT, EXAM_BATCH_SPECS, type ExamSourceBook } from '@/lib/books-ai'
...
async function runGenerationStep(examId: string): Promise<{ ok: boolean; status: string; inserted: number; questionCount: number; totalPoints: number; done: boolean; batchIndex?: number; error?: string }> {
  try {
    ...
    let batch = await generateExamQuestionBatch(exam.program, examKnowledgeBooks, batchIndex, previousTexts)
...
async function runGeneration(examId: string) {
  try {
    ...
    for (let i = startBatch; i < EXAM_BATCH_COUNT; i++) {
      ...
      let batch = await generateExamQuestionBatch(exam.program, examKnowledgeBooks, i)
      if (batch.length === 0) {
        // إعادة محاولة واحدة عند فشل الدفعة
        batch = await generateExamQuestionBatch(exam.program, examKnowledgeBooks, i)
      }
