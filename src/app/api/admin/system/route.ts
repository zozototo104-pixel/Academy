...
const SYSTEM_KEYS = [
  ...
  'USDT_WALLET_ADDRESS', 'USDT_NETWORK', 'USDT_PAYMENT_INSTRUCTIONS',
...
const SECRET_KEYS = new Set([
  'SMTP_PASS', 'RESEND_API_KEY', 'STRIPE_SECRET_KEY', 'STRIPE_WEBHOOK_SECRET', 'PAYPAL_SECRET',
  'GEMINI_API_KEY', 'GEMINI_API_KEYS',
  'OPENAI_API_KEY', 'OPENAI_API_KEYS',
  'ANTHROPIC_API_KEY', 'ANTHROPIC_API_KEYS',
  'ZAI_API_KEY', 'ZAI_API_KEYS',
  'GROQ_API_KEY', 'GROQ_API_KEYS',
  'OPENROUTER_API_KEY', 'OPENROUTER_API_KEYS',
  'DEEPINFRA_API_KEY', 'DEEPINFRA_API_KEYS',
  'TOGETHER_API_KEY', 'TOGETHER_API_KEYS',
  'UNOROUTER_API_KEY', 'UNOROUTER_API_KEYS',
  'RELAYROUTER_API_KEY', 'RELAYROUTER_API_KEYS',
  'OPENAI_COMPAT_API_KEY', 'OPENAI_COMPAT_API_KEYS',
  'AI_AGENT_API_KEY',
])
...
export async function GET() {
  try {
    ...
    const launchReadiness = [
      ...
      { id: 'payment', label: 'الدفع الإلكتروني واليدوي', status: payment.trueGatewayCount > 0 ? 'ok' : payment.usdtConfigured ? 'warn' : payment.errors.length > 0 ? 'error' : 'warn', detail: payment.trueGatewayCount > 0 ? `بوابات الدفع الحقيقية المفعلة: ${payment.trueGatewayCount}.` : payment.usdtConfigured ? `لا توجد بوابة إلكترونية live، لكن دفع USDT اليدوي مضبوط على شبكة ${payment.usdtNetwork || 'TRC20'} وينتظر تأكيد الإدارة.` : payment.errors[0] || 'لا توجد بوابة دفع حقيقية مفعلة حالياً؛ Stripe/PayPal بحاجة لإعداد live أو فعّل USDT/الدفع المباشر يدوياً.' },
      { id: 'gemini', label: 'Gemini / الذكاء الاصطناعي', status: gemini.source === 'none' && !textAi.externalConfigured ? 'warn' : 'ok', detail: textAi.externalConfigured ? `${textAi.message} — Gemini يبقى للصوت/Live أو كاحتياط.` : gemini.source === 'none' ? 'مفتاح Gemini غير موجود؛ وظائف الذكاء الاصطناعي ستكون محدودة.' : `مفتاح Gemini مستخدم من ${gemini.source === 'db' ? 'لوحة الإدارة' : 'Vercel'}.` },
    ...
    return NextResponse.json({
      ...
      secretsSet: {
        ...
        GEMINI_API_KEYS: !!(await db.setting.findUnique({ where: { key: 'GEMINI_API_KEYS' } }))?.value,
        OPENAI_API_KEY: !!(await db.setting.findUnique({ where: { key: 'OPENAI_API_KEY' } }))?.value,
        OPENAI_API_KEYS: !!(await db.setting.findUnique({ where: { key: 'OPENAI_API_KEYS' } }))?.value,
        ANTHROPIC_API_KEY: !!(await db.setting.findUnique({ where: { key: 'ANTHROPIC_API_KEY' } }))?.value,
        ANTHROPIC_API_KEYS: !!(await db.setting.findUnique({ where: { key: 'ANTHROPIC_API_KEYS' } }))?.value,
        ZAI_API_KEY: !!(await db.setting.findUnique({ where: { key: 'ZAI_API_KEY' } }))?.value,
        ZAI_API_KEYS: !!(await db.setting.findUnique({ where: { key: 'ZAI_API_KEYS' } }))?.value,
        GROQ_API_KEY: !!(await db.setting.findUnique({ where: { key: 'GROQ_API_KEY' } }))?.value,
        GROQ_API_KEYS: !!(await db.setting.findUnique({ where: { key: 'GROQ_API_KEYS' } }))?.value,
        OPENROUTER_API_KEY: !!(await db.setting.findUnique({ where: { key: 'OPENROUTER_API_KEY' } }))?.value,
        OPENROUTER_API_KEYS: !!(await db.setting.findUnique({ where: { key: 'OPENROUTER_API_KEYS' } }))?.value,
        DEEPINFRA_API_KEY: !!(await db.setting.findUnique({ where: { key: 'DEEPINFRA_API_KEY' } }))?.value,
        DEEPINFRA_API_KEYS: !!(await db.setting.findUnique({ where: { key: 'DEEPINFRA_API_KEYS' } }))?.value,
        TOGETHER_API_KEY: !!(await db.setting.findUnique({ where: { key: 'TOGETHER_API_KEY' } }))?.value,
