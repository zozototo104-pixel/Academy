  ...
  try {
    ...
    if (pendingInstallments.length > 0) {
      return NextResponse.json(
        { error: `توجد فاتورة دفعة غير مسددة بالفعل (${pendingInstallments[0].invoiceNo}). ادفعها أو راجع الإدارة قبل إنشاء دفعة جديدة.` },
        { status: 400 }
      )
    }

    const availableToInvoice = roundMoney(plan.remainingTuition)
    const requiredInitialRemaining =
      plan.approvedInitialAmount !== null && app.status === 'AWAITING_TUITION'
    ...
    if (requiredInitialRemaining > 0 && Math.abs(amount - requiredInitialRemaining) > 0.009) {
      return NextResponse.json(
        { error: `يجب أن تكون الدفعة الأولى المعتمدة ${requiredInitialRemaining}$ حتى يتم تفعيل التسجيل الدراسي.` },
        { status: 400 }
      )
    }
    if (amount > availableToInvoice) {
      return NextResponse.json({ error: `المبلغ يتجاوز المتبقي غير المفوتر: ${availableToInvoice}$` }, { status: 400 })
    }
